...
function getAll(schema) {
  return [{
    ...
    version: '3',
    previous: '2',
    install: ['ALTER TYPE ' + schema + '.job_state ADD VALUE IF NOT EXISTS \'failed\' AFTER \'cancelled\''],
    uninstall: [
    ...
    version: '4',
    previous: '3',
    install: ['ALTER TABLE ' + schema + '.job ADD COLUMN priority integer not null default(0)', 'ALTER TABLE ' + schema + '.job ALTER COLUMN createdOn SET DATA TYPE timestamptz', 'ALTER TABLE ' + schema + '.job ALTER COLUMN startedOn SET DATA TYPE timestamptz', 'ALTER TABLE ' + schema + '.job ALTER COLUMN completedOn SET DATA TYPE timestamptz'],
    uninstall: ['ALTER TABLE ' + schema + '.job DROP COLUMN priority', 'ALTER TABLE ' + schema + '.job ALTER COLUMN createdOn SET DATA TYPE timestamp', 'ALTER TABLE ' + schema + '.job ALTER COLUMN startedOn SET DATA TYPE timestamp', 'ALTER TABLE ' + schema + '.job ALTER COLUMN completedOn SET DATA TYPE timestamp']
  }, {
    version: '5',
    previous: '4',
    ...
    version: '7',
    previous: '6',
    install: ['CREATE TABLE IF NOT EXISTS ' + schema + '.archive (LIKE ' + schema + '.job)', 'ALTER TABLE ' + schema + '.archive ADD archivedOn timestamptz NOT NULL DEFAULT now()'],
    uninstall: ['DROP TABLE ' + schema + '.archive']
  }];
}